...
export const BattleProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateHeatLevels = useCallback((chunks: Chunk[]): Chunk[] => {
    ...
    const hotCount = Math.max(1, Math.floor(activeChunks.length * 0.15));
    const warmCount = Math.max(2, Math.floor(activeChunks.length * 0.25));
    ...
    const minActivityThreshold = maxActivity * 0.05;
