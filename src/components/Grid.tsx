...
const Grid: React.FC<GridProps> = ({ onCellClick }) => {
  ...
  // Get color for heat map based on heat level (very subtle colors)
  const getHeatMapColor = useCallback((heatLevel: number): string => {
    switch (heatLevel) {
      case 3: // Hot - top level activity chunks (red)
        return 'rgba(255, 40, 40, 0.10)'; // Very subtle red at 10% opacity
      case 2: // Warm - medium activity chunks (yellow/orange)
        return 'rgba(255, 180, 20, 0.10)'; // Very subtle yellow at 10% opacity
      case 1: // Cool - low activity chunks (blue)
        return 'rgba(40, 120, 255, 0.10)'; // Very subtle blue at 10% opacity
      default: // Not active
  ...
  const drawGrid = useCallback(() => {
    ...
    // Restore ship placement hover effect
    if (hoveredCell && !isDragging) {
      const { x, y } = hoveredCell;

      // For each cell the ship would occupy, check if it's in the viewport and draw the hover effect
      for (let i = 0; i < selectedShipSize; i++) {
        ...
        const isValid = isHorizontal
          ? (cellX < CHUNK_SIZE * CELLS_PER_CHUNK)
          : (cellY < CHUNK_SIZE * CELLS_PER_CHUNK);

        if (!isValid) continue;

        const canvasX = cellX * cellSize - viewportPosition.x;
        const canvasY = cellY * cellSize - viewportPosition.y;

        // Only draw if the cell is within the visible canvas
        if (
          canvasX + cellSize >= 0 && canvasX < canvas.width &&
          canvasY + cellSize >= 0 && canvasY < canvas.height
        ) {
      ...
      if (isDragging && dragStart) {
        ...
        if (rect) {
          ...
          if (dx < 5 && dy < 5) {
            ...
            // Otherwise, handle as cell click
            else if (hoveredCell) {
